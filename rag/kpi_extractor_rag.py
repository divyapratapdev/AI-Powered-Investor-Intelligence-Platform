...
class Retriever:
    ...
    def invoke(self, query: str, company: str | None = None, year: int | None = None, top_k: int = 20) -> list:
        ...
        results = (
            self.client.search(search_text=query, top=top_k, filter=filter_expr)
            if filter_expr
            else self.client.search(search_text=query, top=top_k)
        )
...
def retrieve_context(retriever: Retriever, company: str, year: int) -> str:
    query = f"""
    Annual report financial statements, income statement, balance sheet,
    cash flow statement, risks, growth drivers, financial performance
    for {company} fiscal year {year}
    """
    documents = retriever.invoke(query=query, company=company, year=year, top_k=20)
...
- Risk factors and growth drivers should be concise.
...
def extract_financial_metrics(retriever: Retriever, company: str, year: int) -> dict:
    ...
    metrics = get_structured_completion(prompt=prompt, response_model=FinancialMetrics)
...
def main() -> None:
    ...
    results = extract_financial_metrics(retriever=retriever, company=company, year=year)
